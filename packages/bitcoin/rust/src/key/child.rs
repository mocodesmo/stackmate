...
pub fn derive(master_xprv: &str, purpose: &str, account: &str) -> Result<ChildKeys, S5Error> {
  ...
  let root = match ExtendedPrivKey::from_str(master_xprv) {
    Ok(xprv) => xprv,
    Err(_) => return Err(S5Error::new(ErrorKind::OpError,"Xprv-FromString"))
  };
  ...
  let path = match DerivationPath::from_str(&hardened_path) {
    Ok(hdpath) => hdpath,
    Err(_) => return Err(S5Error::new(ErrorKind::OpError,"Derivation-Path-Format"))
  };
  let child_xprv = match root.derive_priv(&secp, &path) {
    Ok(xprv) => xprv,
    Err(_) => return Err(S5Error::new(ErrorKind::OpError,"Derive-XPrv"))
  };
