...
impl ColdCardKeys{
  pub fn from_json_file(path: &str)->Result<ColdCardKeys,S5Error>{
    let file = match File::open(path){
      Ok(file)=>file,
      Err(_)=> return Err(S5Error::new(ErrorKind::OpError,"File-Open-Cold-Card-Json."))
    };
    
    let cckeys: ColdCardKeys = match serde_json::from_reader(file){
      Ok(structure)=>structure,
      Err(_)=>return Err(S5Error::new(ErrorKind::OpError,"JSON-File-To-Struct"))
    };
...
mod tests {
  ...
  fn test_coldcard_watcher() {
    ...
    let cckeys = ColdCardKeys::from_json_file(&path.to_str().unwrap()).unwrap();
    ...
    let config = WalletConfig::default(&bip84_deposit_desc, network);
    ...
    let config = WalletConfig::default(&bip49_deposit_desc, network);
    ...
    let config = WalletConfig::default(&bip44_deposit_desc, network);
