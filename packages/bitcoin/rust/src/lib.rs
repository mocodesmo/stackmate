...
pub unsafe extern "C" fn generate_master(
    ...
    network: *const c_char) -> *mut c_char {
...
#[deny(unsafe_op_in_unsafe_fn)]
#[no_mangle]
pub unsafe extern "C" fn sync_balance(deposit_desc: *const c_char, network: *const c_char)-> *mut c_char {
...
pub unsafe extern "C" fn get_address(
    ...
    let address_index: u32 = match index_cstr.to_str() {
        Ok(string) => match string.parse::<u32>(){
            Ok(i)=>i,
            Err(_)=>return CString::new("Error: Address Index Input.").unwrap().into_raw()
        },
...
mod tests {
    ...
    fn test_c_master_ops(){
        ...
        unsafe{
            ...
            let master = generate_master(CString::new("9").unwrap().into_raw(),CString::new("").unwrap().into_raw(),CString::new("tpanini").unwrap().into_raw());
            let master = CStr::from_ptr(master).to_str().unwrap();
            let master: master::MasterKey = serde_json::from_str(master).unwrap();
            assert_eq!(24,master.mnemonic.split_whitespace().collect::<Vec<&str>>().len());
            ...
            let master = import_master(CString::new(mnemonic).unwrap().into_raw(),CString::new("").unwrap().into_raw(),CString::new("tpanini").unwrap().into_raw());
            let master = CStr::from_ptr(master).to_str().unwrap();
            let master: master::MasterKey = serde_json::from_str(master).unwrap();
 ...
 */
    ...
    fn test_c_wallet(){
        unsafe{
            ...
            assert_eq!(address.address,"tb1q5f3jl5lzlxtmhptfe9crhmv4wh392ku5ztkpt6xxmqqx2c3jyxrs8vgat7");
        }
    }
}
